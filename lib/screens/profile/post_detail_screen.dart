...
class _PostDetailScreenState extends State<PostDetailScreen>
  ...
  void initState() {
    ...
    if (widget.autoFocusComment) {
      WidgetsBinding.instance
          .addPostFrameCallback((_) => _commentFocus.requestFocus());
    }
  }
  ...
  // ── Like dengan notifikasi ─────────────────────────────────────────────────

  Future<void> _toggleLike() async {
    final postOwnerUid  = widget.post['uid'] as String? ?? '';
    final postCaption   = widget.post['caption'] as String? ?? '';

    // Cek status like sebelum toggle
    final wasLiked =
        await _postService.isLikedStream(widget.postId).first;

    await _postService.toggleLike(widget.postId);

    // Kirim notif hanya saat baru like (bukan unlike)
  ...
  Future<void> _sendComment() async {
    ...
    try {
      ...
      // Kirim notif komentar ke pemilik postingan
      if (postOwnerUid.isNotEmpty) {
        NotificationService.instance.sendCommentNotification(
          postOwnerUid  : postOwnerUid,
          commenterName : _myName,
        ...
        child: photo.isNotEmpty
            ...
            : Container(
                ...
                child: Icon(Icons.person,
                    color: Colors.purple, size: size * 0.5)),
      ),
    );
  }
