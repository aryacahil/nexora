...
// ─────────────────────────────────────────────────────────────────────────────
// Custom Painter: Pistol (tidak berubah)
// ─────────────────────────────────────────────────────────────────────────────
...
class PistolPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final darkPaint = Paint()..color = color.withValues(alpha: 0.6)..style = PaintingStyle.fill;
    final highlightPaint = Paint()..color = Colors.white.withValues(alpha: 0.25)..style = PaintingStyle.fill;
    ...
    if (glowIntensity > 0) {
      ...
      canvas.drawRRect(RRect.fromRectAndRadius(
        ...
        const Radius.circular(20)), glowPaint);
    }
    canvas.drawRRect(RRect.fromRectAndRadius(
      Rect.fromLTWH(w * 0.55, h * 0.28, w * 0.42, h * 0.18),
      const Radius.circular(4)), bodyPaint);
    canvas.drawRRect(RRect.fromRectAndRadius(
      Rect.fromLTWH(w * 0.93, h * 0.25, w * 0.05, h * 0.24),
      const Radius.circular(2)), darkPaint);
    canvas.drawRRect(RRect.fromRectAndRadius(
      Rect.fromLTWH(w * 0.2, h * 0.22, w * 0.38, h * 0.32),
      const Radius.circular(6)), bodyPaint);
    canvas.drawRRect(RRect.fromRectAndRadius(
      Rect.fromLTWH(w * 0.22, h * 0.24, w * 0.34, h * 0.08),
      const Radius.circular(4)), highlightPaint);
    canvas.drawCircle(Offset(w * 0.42, h * 0.38), w * 0.13,
      Paint()..color = color.withValues(alpha: 0.85)..style = PaintingStyle.fill);
    final holePaint = Paint()..color = Colors.black.withValues(alpha: 0.55)..style = PaintingStyle.fill;
    final holeBorderPaint = Paint()..color = Colors.white.withValues(alpha: 0.15)..style = PaintingStyle.stroke..strokeWidth = 1;
    for (int i = 0; i < 6; i++) {
    ...
    final gripPath = Path()
      ..moveTo(w * 0.22, h * 0.52)..lineTo(w * 0.35, h * 0.52)
      ..lineTo(w * 0.30, h * 0.82)..lineTo(w * 0.16, h * 0.82)
      ..lineTo(w * 0.14, h * 0.65)..close();
    canvas.drawPath(gripPath, Paint()..color = color.withValues(alpha: 0.75)..style = PaintingStyle.fill);
    final gripTexturePaint = Paint()..color = Colors.black.withValues(alpha: 0.2)..strokeWidth = 1.2..style = PaintingStyle.stroke;
    for (int i = 0; i < 5; i++) {
      final y = h * 0.57 + i * h * 0.045;
      canvas.drawLine(Offset(w * 0.16, y), Offset(w * 0.32, y + h * 0.01), gripTexturePaint);
    }
    canvas.drawPath(
      Path()..moveTo(w * 0.28, h * 0.54)..quadraticBezierTo(w * 0.28, h * 0.70, w * 0.18, h * 0.68),
      Paint()..color = color.withValues(alpha: 0.7)..style = PaintingStyle.stroke..strokeWidth = w * 0.03..strokeCap = StrokeCap.round);
    canvas.drawLine(Offset(w * 0.265, h * 0.57), Offset(w * 0.245, h * 0.65),
      Paint()..color = darkPaint.color..style = PaintingStyle.stroke..strokeWidth = w * 0.025..strokeCap = StrokeCap.round);
    canvas.drawRRect(RRect.fromRectAndRadius(
  ...
  @override
  bool shouldRepaint(PistolPainter old) =>
  ...
  Widget _buildRules() {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _rule(Icons.meeting_room_outlined, 'Buat atau gabung room dengan kode'),
        _rule(Icons.people_outline, 'Tunggu minimal 2 pemain, lalu mulai'),
        _rule(Icons.ads_click, 'Setiap giliran, pemain menarik pelatuk revolver'),
        _rule(Icons.casino_outlined, '1 dari 6 kemungkinan peluru akan mengenaimu'),
        _rule(Icons.emoji_events_outlined, 'Pemain terakhir yang selamat adalah pemenang!'),
      ]),
    );
  }
              ...
              child: Container(
                ...
                child: Row(children: [
                  ...
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    Text('${players.length}/6', style: TextStyle(
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: isFull ? AppColors.border : _red,
                      borderRadius: BorderRadius.circular(20)),
...
// ─────────────────────────────────────────────────────────────────────────────
// Room Screen (tidak berubah — hanya dipindah ke bawah)
// ─────────────────────────────────────────────────────────────────────────────
...
class _RoomScreenState extends State<_RoomScreen>
    ...
    _recoilAnimation = TweenSequence([
      ...
      TweenSequenceItem(
        tween: Tween<double>(begin: -20, end: 0)
            .chain(CurveTween(curve: Curves.elasticOut)), weight: 75),
  ...
  Future<void> _playSound(String type) async {
    try {
      switch (type) {
        case 'spin':     await _audioPlayer.play(AssetSource('sounds/spin.wav'));    break;
        case 'click':    await _audioPlayer.play(AssetSource('sounds/click.wav'));   break;
        case 'gunshot':  await _audioPlayer.play(AssetSource('sounds/gunshot.wav')); break;
        case 'win':      await _audioPlayer.play(AssetSource('sounds/win.wav'));     break;
      }
    } catch (_) {}
  }
  ...
  Future<void> _pullTrigger() async {
    ...
    final bulletPos    = data['bulletPosition'] as int;
    final isShot       = triggerCount == bulletPos;
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final alivePlayers = players
            .where((p) => p['isAlive'] == true).toList();
        final isHost      = players.any(
            (p) => p['uid'] == widget.myUid && p['isHost'] == true);
        final myPlayer    = players.firstWhere(
            (p) => p['uid'] == widget.myUid, orElse: () => {});
            ...
            title: Row(children: [
              Text('ROOM: ${widget.roomCode}',
                style: TextStyle(fontSize: 15, fontWeight: FontWeight.w900,
                  color: Colors.red.shade400, letterSpacing: 2)),
              const SizedBox(width: 10),
              Container(
                padding: const EdgeInsets.symmetric(
                ...
                decoration: BoxDecoration(
                  color: status == 'playing'
                      ? Colors.green.withValues(alpha: 0.15) : AppColors.border,
                  borderRadius: BorderRadius.circular(8)),
                child: Text(
                  status == 'waiting' ? 'LOBBY'
                      : status == 'playing' ? 'LIVE' : 'SELESAI',
                  style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900,
                    letterSpacing: 1.5,
                    color: status == 'playing'
                        ? Colors.green.shade400 : AppColors.textDim)),
              ),
            ]),
          ),
          body: Stack(children: [
              ...
              children: [
                ...
                if (status == 'finished') ...[
                  Container(
                    ...
                    child: Column(children: [
                      ...
                      const Text('PEMENANG!', style: TextStyle(
                        color: Colors.white70, fontSize: 12, letterSpacing: 3)),
                      ...
                      if (isHost)
                        GestureDetector(
                          onTap: () => widget.gameService
                              .restartGame(widget.roomCode),
                          child: Container(
                            padding: const EdgeInsets.symmetric(
                        ...
                        builder: (context, _) => Transform.translate(
                          ...
                          child: ScaleTransition(
                            ...
                            child: SizedBox(
                              ...
                              child: CustomPaint(painter: PistolPainter(
                                color: isMyTurn
                                    ? Colors.red.shade400
                        ...
                        child: Container(
                          ...
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            Icon(
                              ...
                              size: 13,
                              color: isMyTurn
                                  ? Colors.red.shade400 : Colors.white38),
                  ...
                  if (isMyTurn)
                    GestureDetector(
                      ...
                      child: AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          gradient: LinearGradient(colors: _isShooting
                        ...
                        child: _isShooting
                            ...
                            : const Row(
                                ...
                                children: [
                                  ...
                                  SizedBox(width: 10),
                                  Text('TARIK PELATUK', style: TextStyle(
                ...
                ...players.map((p) {
                  ...
                  return Container(
                    ...
                    decoration: BoxDecoration(
                      color: isCurrentTurn
                          ? Colors.red.withValues(alpha: 0.12) : AppColors.card,
                      borderRadius: BorderRadius.circular(16),
                      border: Border.all(
                        color: isCurrentTurn
                            ? Colors.red.shade400 : AppColors.border,
                    ...
                    child: Row(children: [
                      Container(
                        ...
                        child: Icon(
                          alive
                              ? (isCurrentTurn
                                  ? Icons.radio_button_checked : Icons.person)
                              : Icons.person_off,
                          color: alive
                              ? (isCurrentTurn
                                  ? Colors.red.shade400 : AppColors.primary)
                              : Colors.grey,
                          size: 22)),
                      const SizedBox(width: 12),
                      Expanded(child: Column(
                        ...
                        children: [
                          Row(children: [
                            Text(p['name'] ?? '', style: TextStyle(
                              ...
                              color: alive ? AppColors.textMain : AppColors.textDim,
                            ...
                            if (isMe) ...[
                              ...
                              Container(
                                ...
                                child: Text('KAMU', style: TextStyle(
                          ...
                          Row(children: [
                            Icon(
                              ...
                              size: 12,
                              color: isCurrentTurn
                                  ? Colors.red.shade400 : AppColors.textDim),
                      ...
                      decoration: BoxDecoration(
                        gradient: players.length >= 2
                            ? const LinearGradient(colors: [
                                Color(0xFF7C0000), Color(0xFFDC2626)])
                            : null,
                        ...
                        children: [
                          Icon(
                            ...
                            color: players.length >= 2
                                ? Colors.white : AppColors.textDim, size: 20),
                          const SizedBox(width: 10),
                          Text(
                            players.length < 2
                                ? 'Menunggu pemain lain...' : 'MULAI GAME',
                            style: TextStyle(
                              color: players.length >= 2
  ...
  Widget _buildLatestLog(String text) {
    ...
    if (text.startsWith('MATI:'))       { logIcon = Icons.close_rounded;       logColor = Colors.red;        displayText = text.substring(5).trim(); }
    else if (text.startsWith('SELAMAT:')) { logIcon = Icons.check_circle_outline; logColor = Colors.greenAccent; displayText = text.substring(8).trim(); }
    else if (text.startsWith('MENANG:')) { logIcon = Icons.emoji_events;         logColor = Colors.amber;      displayText = text.substring(7).trim(); }
    else if (text.startsWith('RESET:'))  { logIcon = Icons.refresh;              logColor = Colors.blueAccent; displayText = text.substring(6).trim(); }
    else if (text.startsWith('MULAI:'))  { logIcon = Icons.play_circle_outline;  logColor = Colors.white54;    displayText = text.substring(6).trim(); }
    else                                  { logIcon = Icons.info_outline;          logColor = Colors.white54;    displayText = text; }
